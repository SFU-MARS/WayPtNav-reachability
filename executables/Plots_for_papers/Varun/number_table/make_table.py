...
end_to_end_dir = '/home/ext_drive/somilb/data/sessions/sbpd/rgb/sbpd_projected_grid/nn_control/resnet_50_v1/' \
                 'include_last_step/only_successful_episodes/session_2019-01-27_23-34-22/test/checkpoint_18/' \
                 'clean_codebase_500_goals/session_2019-04-08_09-56-57/rgb_resnet50_nn_control_simulator/trajectories'
waypoint_dir = '/home/ext_drive/somilb/data/sessions/sbpd/rgb/sbpd_projected_grid/nn_waypoint/resnet_50_v1/' \
               'include_last_step/only_successful_episodes/training_continued_from_epoch9/' \
               'session_2019-01-27_23-32-01/test/checkpoint_9/clean_codebase_500_goals/session_2019-04-05_14-26-07/' \
               'rgb_resnet50_nn_waypoint_simulator/trajectories'
expert_dir = '/home/ext_drive/somilb/data/sessions/expert/sbpd_area1_500_goals_small_obs_margin/' \
             'session_2019-06-28_13-29-44/expert_simulator/trajectories/'

mapper_dir = '/home/ext_drive/somilb/data/sessions/classical_pipeline/checkpoint_9/map_and_freespace_based_planner/' \
             'small_obs_margin/map_500_goals/session_2019-06-26_12-40-48/classical_simulator/trajectories'
reactive_dir = '/home/ext_drive/somilb/data/sessions/classical_pipeline/checkpoint_9/map_and_freespace_based_planner/' \
               'small_obs_margin/freespace_500_goals/session_2019-06-26_12-38-27/classical_simulator/trajectories'
...
def get_bar_chart_data(return_expert_data=False):
    ...
    end_to_end_common_episode_lengths = compute_metric_for_goal_numbers(end_to_end_dir, common_success_goals, 'episode_length')
    waypoint_common_episode_lengths = compute_metric_for_goal_numbers(waypoint_dir, common_success_goals, 'episode_length')
    
    # Compute the Average Acceleration Magnitude for this common set of goals
    end_to_end_common_avg_accelerations = compute_metric_for_goal_numbers(end_to_end_dir, common_success_goals, 'avg_acceleration')
    waypoint_common_avg_accelerations = compute_metric_for_goal_numbers(waypoint_dir, common_success_goals, 'avg_acceleration')
    ...
    e2e_data = [end_to_end_percentages, end_to_end_common_episode_lengths, end_to_end_common_avg_accelerations, end_to_end_common_avg_jerks]
    waypt_data = [waypoint_percentages, waypoint_success_goals, waypoint_common_episode_lengths, waypoint_common_avg_accelerations, waypoint_common_avg_jerks]
    ...
    if return_expert_data:
        ...
        expert_percentages = {'Success' : 1.0,
                              'Collision' : 0.0,
                              'Timeout' : 0.0}
        expert_common_episode_lengths = compute_metric_for_goal_numbers(expert_dir, common_success_goals, 'episode_length')
        expert_common_avg_accelerations = compute_metric_for_goal_numbers(expert_dir, common_success_goals, 'avg_acceleration')
        expert_common_avg_jerks = compute_metric_for_goal_numbers(expert_dir, common_success_goals, 'avg_jerk')
        expert_data = [expert_percentages, expert_common_episode_lengths, expert_common_avg_accelerations, expert_common_avg_jerks]
...
def make_bar_chart_v0():
    ...
    rects_waypoints = ax.bar(idx+bar_width, waypoint_data/scale_factor, bar_width, color='b', alpha=.8, label='Waypoint')

    plt.xticks(idx + bar_width/2., ['% Success', 'Average Time to Reach Goal', 'Average Acceleration', 'Average Jerk'],
...
def make_bar_chart_v1(x_label_fontsize=12, bar_fontsize=12, legend_size=14, error_bars_mode='std'):
    ...
    x_labels = ['Success (%)\n(A)', 'Average Time to Reach Goal (s)\n(B)', r'Average Acceleration (m/s$^2$)' '\n(C)', r'Average Jerk (m/s$^3$)' '\n(D)']

    for i, (end_to_end_datum, waypoint_datum, x_label, e2e_error_bar, waypoint_error_bar) in enumerate(zip(end_to_end_data, waypoint_data, x_labels, end_to_end_error_bars, waypoint_error_bars)):
        ...
        rects_waypoint = ax.bar(0, waypoint_datum, bar_width, color=waypoint_color, alpha=waypoint_alpha, **kwargs_waypoint)
        rects_end_to_end = ax.bar(bar_width, end_to_end_datum, bar_width, color=end_to_end_color, alpha=end_to_end_alpha, **kwargs_e2e)
...
def make_bar_chart_v2(x_label_fontsize=12, bar_fontsize=12, legend_size=14, error_bars_mode='std'):
    ...
    x_labels = ['Success (%)', 'Average Time to Reach Goal (s)', r'Average Acceleration (m/s$^2$)', r'Average Jerk (m/s$^3$)']
    ...
    for i, (end_to_end_datum, waypoint_datum, x_label, e2e_error_bar, waypoint_error_bar) in enumerate(zip(end_to_end_data, waypoint_data, x_labels, end_to_end_error_bars, waypoint_error_bars)):
        ...
        bar_width = .1
        bar_spacing = .12
        rects_waypoint = ax.barh(bar_spacing, waypoint_datum, height=bar_width, color=waypoint_color, alpha=waypoint_alpha, **kwargs_waypoint)
        rects_end_to_end = ax.barh(0, end_to_end_datum, height=bar_width, color=end_to_end_color, alpha=end_to_end_alpha, **kwargs_e2e)
...
def make_bar_chart_v3(x_label_fontsize=12, bar_fontsize=12,
    ...
    zipped_data = zip(end_to_end_data, waypoint_data, expert_data, x_labels, end_to_end_error_bars, waypoint_error_bars, expert_error_bars)
    for i, (end_to_end_datum, waypoint_datum, expert_datum, x_label, e2e_error_bar, waypoint_error_bar, expert_error_bar) in enumerate(zipped_data):
        ...
        rects_waypoint = ax.barh(3*bar_spacing, waypoint_datum, height=bar_width, color=waypoint_color, alpha=waypoint_alpha, **kwargs_waypoint)
        rects_end_to_end = ax.barh(2*bar_spacing , end_to_end_datum, height=bar_width, color=end_to_end_color, alpha=end_to_end_alpha, **kwargs_e2e)
        rects_expert = ax.barh(bar_spacing , expert_datum, height=bar_width, color=expert_color, alpha=expert_alpha, **kwargs_expert)
...
def report_numbers():
    ...
    end_to_end_data, waypoint_data, mapper_data, reactive_data, expert_data = get_bar_chart_data_v2(return_expert_data=True)
    ...
    # Time 
    print('Time to reach')
    print('method: expert', 'mean', np.mean(expert_common_episode_lengths), 'std', np.std(expert_common_episode_lengths))
    print('method: waypoint', 'mean', np.mean(waypoint_common_episode_lengths), 'std', np.std(waypoint_common_episode_lengths))
    print('method: E2E', 'mean', np.mean(end_to_end_common_episode_lengths), 'std', np.std(end_to_end_common_episode_lengths))
    print('method: map', 'mean', np.mean(mapper_common_episode_lengths), 'std', np.std(mapper_common_episode_lengths))
    print('method: reactive', 'mean', np.mean(reactive_common_episode_lengths), 'std', np.std(reactive_common_episode_lengths))
    
    
    # Acceleration
    print('Acceleration')
    print('method: expert', 'mean', np.mean(expert_common_avg_accelerations), 'std', np.std(expert_common_avg_accelerations))
    print('method: waypoint', 'mean', np.mean(waypoint_common_avg_accelerations), 'std', np.std(waypoint_common_avg_accelerations))
    print('method: E2E', 'mean', np.mean(end_to_end_common_avg_accelerations), 'std', np.std(end_to_end_common_avg_accelerations))
    print('method: map', 'mean', np.mean(mapper_common_avg_accelerations), 'std', np.std(mapper_common_avg_accelerations))
    print('method: reactive', 'mean', np.mean(reactive_common_avg_accelerations), 'std', np.std(reactive_common_avg_accelerations))
